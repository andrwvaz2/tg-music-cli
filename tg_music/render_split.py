...
class RenderSplitMixin:
    ...
    def draw_split(self) -> None:
        ...
        tracks_to_show = (
            self.tracks if self.channel_filter is None else [t for t in self.tracks if t.channel == self.channel_filter]
        )
        ...
        for row_idx, track in enumerate(tracks_to_show[track_offset : track_offset + body_h]):
            ...
            is_caching = (track.id in self.precache_ids and not track.local_path) or (
    ...
    def draw_split_details(self, top: int, x: int, width: int, height: int) -> None:
        ...
        if self.current_track.performer:
            if row >= top + height:
                return
            self.screen.addnstr(
                row, x + 1, self.current_track.performer[: max(width - 2, 0)], max(width - 2, 0), curses.A_NORMAL
            )
            row += 1
        ...
        if self.play_start_time is not None and row < top + height:
            ...
            self.screen.addnstr(
                row,
                x + 1,
                f"{elapsed_str} {bar} {dur_str}"[: max(width - 2, 0)],
                max(width - 2, 0),
                self.color_attr(self.color_success, -1),
            )
            row += 1
        ...
        if self.cover_graphics and cover_h >= 2:
            ...
            for blank_row in range(row, row + blank_rows):
                self.screen.addnstr(
                    blank_row, cover_x, " " * inner_width, max(width - (cover_x - x) - 1, 0), curses.A_DIM
                )
            if blank_rows > 0:
            ...
            self.screen.addnstr(
                row, x + 1, "\u250c" + "\u2500" * (cover_w - 2) + "\u2510", max(width - 2, 0), curses.A_DIM
            )
            row += 1
        ...
        if self.play_queue and row < top + height:
            self.screen.addnstr(
                row, x + 1, "Queue:", max(width - 2, 0), self.color_attr(self.color_primary, -1) | curses.A_BOLD
            )
            row += 1
            ...
            for qid in self.play_queue[: max(0, top + height - row)]:
                ...
                if qt:
                    self.screen.addnstr(
                        row, x + 1, f"-> {qt.display_title}"[: max(width - 2, 0)], max(width - 2, 0), curses.A_DIM
                    )
                    row += 1
